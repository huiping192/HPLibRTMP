...
@interface HPStreamingBuffer (){
    dispatch_semaphore_t _lock;
}

@property (nonatomic, strong) NSMutableArray <HPFrame *> *sortList;
@property (nonatomic, strong, readwrite) NSMutableArray <HPFrame *> *list;
...
@implementation HPStreamingBuffer
...
#pragma mark -- Custom
- (void)appendObject:(HPFrame *)frame {
    if (!frame) return;
    if (!_startTimer) {
        _startTimer = YES;
        [self tick];
    ...
    if (self.sortList.count < defaultSortBufferMaxCount) {
        ...
        /// 添加至缓冲区
        HPFrame *firstFrame = [self.sortList lfPopFirstObject];
...
- (HPFrame *)popFirstObject {
    dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
    HPFrame *firstFrame = [self.list lfPopFirstObject];
    dispatch_semaphore_signal(_lock);
    return firstFrame;
}
...
- (NSArray *)expirePFrames {
    ...
    for (NSInteger index = 0; index < self.list.count; index++) {
        HPFrame *frame = [self.list objectAtIndex:index];
        if ([frame isKindOfClass:[HPVideoFrame class]]) {
            HPVideoFrame *videoFrame = (HPVideoFrame *)frame;
...
- (NSArray *)expireIFrames {
    ...
    for (NSInteger index = 0; index < self.list.count; index++) {
        HPFrame *frame = [self.list objectAtIndex:index];
        if ([frame isKindOfClass:[HPVideoFrame class]] && ((HPVideoFrame *)frame).isKeyFrame) {
...
NSInteger frameDataCompare(id obj1, id obj2, void *context){
    HPFrame *frame1 = (HPFrame *)obj1;
    HPFrame *frame2 = (HPFrame *)obj2;
...
- (HPLiveBuffferState)currentBufferState {
    ...
    if (increaseCount >= self.callBackInterval) {
        return HPLiveBuffferStateIncrease;
    }

    if (decreaseCount >= self.callBackInterval) {
        return HPLiveBuffferStateDecline;
    }
    
    return HPLiveBuffferStateUnknown;
}
...
- (void)tick {
    ...
    if (self.currentInterval >= self.callBackInterval) {
        HPLiveBuffferState state = [self currentBufferState];
        if (state == HPLiveBuffferStateIncrease) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(streamingBuffer:bufferState:)]) {
                [self.delegate streamingBuffer:self bufferState:HPLiveBuffferStateIncrease];
            }
        } else if (state == HPLiveBuffferStateDecline) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(streamingBuffer:bufferState:)]) {
                [self.delegate streamingBuffer:self bufferState:HPLiveBuffferStateDecline];
            }
        }
