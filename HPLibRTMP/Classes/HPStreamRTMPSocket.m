...
static const PILI_AVal av_SDKVersion = AVC("HPLiveRTMP 1.0.0");
SAVC(onMetaData);
SAVC(duration);
SAVC(width);
SAVC(height);
...
@interface HPStreamRTMPSocket ()<HPStreamingBufferDelegate>
{
    PILI_RTMP *_rtmp;
}
@property (nonatomic, strong) HPLiveStreamInfo *stream;
@property (nonatomic, strong) HPStreamingBuffer *buffer;
@property (nonatomic, strong) HPLiveDebug *debugInfo;
@property (nonatomic, strong) dispatch_queue_t rtmpSendQueue;

@property (nonatomic, weak) id<HPStreamSocketDelegate> _Nullable delegate;
...
@implementation HPStreamRTMPSocket
...
- (void)_start {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:HPLiveStatePending];
    }
...
- (void)_stop {
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:HPLiveStateStop];
    }
    if (_rtmp != NULL) {
        PILI_RTMP_Close(_rtmp, &_error);
        PILI_RTMP_Free(_rtmp);
    ...
    __weak typeof(self) _self = self;
     dispatch_async(self.rtmpSendQueue, ^{
        if (!_self.isSending && _self.buffer.list.count > 0) {
            ...
            // 调用发送接口
            HPFrame *frame = [_self.buffer popFirstObject];
            if ([frame isKindOfClass:[HPVideoFrame class]]) {
                if (!_self.sendVideoHead) {
                    _self.sendVideoHead = YES;
                    if(!((HPVideoFrame*)frame).sps || !((HPVideoFrame*)frame).pps){
                        _self.isSending = NO;
                        return;
                    }
                    [_self sendVideoHeader:(HPVideoFrame *)frame];
                } else {
                    [_self sendVideo:(HPVideoFrame *)frame];
                }
            } else {
                if (!_self.sendAudioHead) {
                    _self.sendAudioHead = YES;
                    if(!((HPAudioFrame*)frame).audioInfo){
                        _self.isSending = NO;
                        return;
                    }
                    [_self sendAudioHeader:(HPAudioFrame *)frame];
                } else {
            ...
            if (_self.debugInfo.elapsedMilli < 1000) {
                ...
                if ([frame isKindOfClass:[HPAudioFrame class]]) {
...
- (NSInteger)RTMP264_Connect:(char *)push_url {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:HPLiveStateStart];
    }
...
- (void)sendMetaData {
    ...
    *enc++ = PILI_AMF_OBJECT;
    ...
    // videosize
    enc = PILI_AMF_EncodeNamedNumber(enc, pend, &av_width, self.stream.videoSize.width);
    enc = PILI_AMF_EncodeNamedNumber(enc, pend, &av_height, self.stream.videoSize.height);

    // video
    enc = PILI_AMF_EncodeNamedString(enc, pend, &av_videocodecid, &av_avc1);

    enc = PILI_AMF_EncodeNamedNumber(enc, pend, &av_videodatarate, self.stream.videoBitrate / 1000.f);
    enc = PILI_AMF_EncodeNamedNumber(enc, pend, &av_framerate, self.stream.videoFrameRate);

    // audio
    enc = PILI_AMF_EncodeNamedString(enc, pend, &av_audiocodecid, &av_mp4a);
    enc = PILI_AMF_EncodeNamedNumber(enc, pend, &av_audiodatarate, self.stream.audioBitrate);

    enc = PILI_AMF_EncodeNamedNumber(enc, pend, &av_audiosamplerate, self.stream.audioSampleRate);
    enc = PILI_AMF_EncodeNamedNumber(enc, pend, &av_audiosamplesize, 16.0);
    enc = PILI_AMF_EncodeNamedBoolean(enc, pend, &av_stereo, self.stream.numberOfChannels == 2);
...
- (void)sendVideoHeader:(HPVideoFrame *)videoFrame {
...
- (void)sendVideo:(HPVideoFrame *)frame {
...
- (void)sendAudioHeader:(HPAudioFrame *)audioFrame {
...
- (void)reconnect {
    dispatch_async(self.rtmpSendQueue, ^{
        if (self.retryTimes4netWorkBreaken++ < self.reconnectCount && !self.isReconnecting) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
                [self.delegate socketStatus:self status:HPLiveStateError];
            }
            if (self.delegate && [self.delegate respondsToSelector:@selector(socketDidError:errorCode:)]) {
                [self.delegate socketDidError:self errorCode:HPLiveSocketErrorCodeReConnectTimeOut];
            }
        }
    });
}
...
- (void)_reconnect{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:HPLiveStateRefresh];
    }
...
- (HPStreamingBuffer *)buffer {
    if (!_buffer) {
        _buffer = [[HPStreamingBuffer alloc] init];
...
- (HPLiveDebug *)debugInfo {
    if (!_debugInfo) {
        _debugInfo = [[HPLiveDebug alloc] init];
    }
    return _debugInfo;
}
